...
export const addVisitedLocation = async (location) => {
    try {
...
export const updateStepCount = async (steps) => {
    try {
...
const checkVisitPetAchievement = async (count) => {
    if (count >= 100) {
...
const checkTravelAllLocationsAchievement = async (locations) => {
...
const checkUseInsuranceAchievement = async (count) => {
    if (count >= 10) {
...
const checkWalkMillionStepsAchievement = async (steps) => {
    if (steps >= 1000000) {
...
const markAchievementComplete = async (achievementName) => {
    try {
        const achievements = await AsyncStorage.getItem('achievements');
        if (achievements) {
            const parsedAchievements = JSON.parse(achievements);
            const updatedAchievements = parsedAchievements.map((achievement) => {
...
export const checkCoinSpendingAchievements = async (coinsSpent) => {
    try {
...
export const checkBackInShapeAchievement = async (previousHealthValue, currentHealthValue) => {
    try {
