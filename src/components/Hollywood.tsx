...
// Back button handler
const handleBackButton = async (navigation: StackNavigationProp<RootStackParamList, 'Hollywood'>) => {
...
// HollywoodScreen component
const HollywoodScreen: React.FC<HollywoodScreenProps> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    addVisitedLocation('Hollywood'); // or 'Osaka'
  }, []);
    ...
    return (
      <View style={styles.cosmeticsContainer}>
        {cosmeticItems.map((item, index) => {
          ...
          // Determine the correct style based on the item type
          ...
          // Only render if a valid style was found
